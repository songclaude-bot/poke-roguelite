...
## Phase 27: 전투 비주얼 + 완료 추적

### 27-1. 데미지 숫자 개선 (Phase 257)
- 날짜: 2026-02-24
- 효과 색상: 초효과(빨강), 일반(흰색), 약효과(회색), 면역, 크리티컬(금색)
- 데미지 크기 스케일링 (10-16px), 바운스 애니메이션
- 11종 타입 색상 히트 스파크 효과
- 크리티컬 금색 플래시, 4줄 전투 로그

### 27-2. 던전 완료 추적 (Phase 258)
- 날짜: 2026-02-24
- clearedDungeons 배열로 클리어 던전 추적
- 던전 목록에 초록 체크마크(✓)
- 티어별 완료 카운트 + 금색 COMPLETE 배지
- 전체 진행률 바 (X/217) + 올 클리어 배너

### v3.5.0 마일스톤
- **데미지 비주얼**: 색상/크기 코딩 + 히트 스파크 + 크리티컬 플래시
- **완료 추적**: 체크마크 + 티어 진행 + 전체 프로그레스
- **라이브**: https://songclaude-bot.github.io/poke-roguelite/

---

## Phase 28: 방치형 수입 + 스피드런

### 28-1. 패시브 인컴 시스템 (Phase 260)
- 날짜: 2026-02-24
- 방치 시간에 따른 골드 자동 축적 (기본 10G/시간)
- 클리어 수, NG+ 레벨에 따른 보너스 (최대 24시간 누적)
- 허브 복귀 시 "Welcome Back" 팝업 + 애니메이션 카운터
- 현재 수입률 표시 (+XG/hr)

### 28-2. 던전 스피드런 타이머 (Phase 261)
- 날짜: 2026-02-24
- MM:SS 형식 타이머 HUD (메뉴 중 일시정지)
- 층 이동 시 시간 유지, bestTimes 기록
- 클리어 시 "New Best Time!" 금색 텍스트
- 던전 프리뷰에 최고 기록 표시

### v3.6.0 마일스톤
- **방치형 수입**: 시간 기반 골드 축적 + 보너스 스케일링
- **스피드런**: MM:SS 타이머 + 최고 기록 추적
- **라이브**: https://songclaude-bot.github.io/poke-roguelite/
...
### 29-1. 미니맵 개선 (Phase 263)
- 날짜: 2026-02-24
- 확장형 풀맵 오버레이 (4px/타일, 탭으로 닫기)
- 3단계 전장의 안개 (보이는/탐험됨/미탐험)
- 탐색률 표시 (100%시 금색 "FULLY EXPLORED")
- 색상 범례 아이콘 (초록=플레이어, 빨강=적, 노랑=아이템, 파랑=계단)

### 29-2. 날씨 진화 시스템 (Phase 264)
- 날짜: 2026-02-24
- 4단계 날씨 강도 (Mild/Normal/Intense/Extreme)
- 층 진행에 따른 강도 자동 상승
- 10턴마다 날씨 전환 확률 체크 (5~20%)
- 타입 시너지 보너스 (+15% 데미지)
- 색상 코딩 HUD (흰/노/주/빨)

### v3.7.0 마일스톤
- **미니맵**: 풀맵 오버레이 + 안개 시스템 + 탐색률
- **날씨 진화**: 4단계 강도 + 전환 이벤트 + 타입 시너지
- **라이브**: https://songclaude-bot.github.io/poke-roguelite/

---

## Phase 30: 해저드 타일 + 점수 체인

### 30-1. 던전 해저드 타일 (Phase 266)
- 날짜: 2026-02-24
- 6종 해저드: Lava(8dmg), Water(감속), ToxicSwamp(5dmg+독), IcePatch(미끄러짐), Quicksand(3dmg+속박), ElectricFloor(6dmg+마비)
- 타입별 면역 시스템 (Fire→Lava, Ice→IcePatch 등)
- 층 깊이에 따른 밀도 스케일링 (5-15%)
- 해저드별 고유 시각 효과 (맥동, 반짝, 진동 등)
- 미니맵에 해저드 컬러 표시

### 30-2. 점수 배율 체인 시스템 (Phase 267)
- 날짜: 2026-02-24
- C→B→A→S→SS→SSS 6단계 체인 티어
- 킬(+0.1x), 효과적 공격(+0.05x), 몬스터하우스 클리어(+0.5x), 아이템 획득(+0.02x), 빠른 층 통과(+0.2x)
- 피격/힐/5턴 방치 시 체인 리셋 (최대 5.0x)
- 체인 HUD + 티어업 애니메이션 (스케일 팝 + 플래시)
- 클리어/게임오버 화면에 최고 체인 + 보너스 점수
...
## Phase 32: 보스 가운틀릿 + 켈레온 상점

### 32-1. 보스 가운틀릿 시스템 (Phase 272)
- 날짜: 2026-02-24
- 멀티 웨이브 보스전: 일반 던전(2웨이브), 보스러시(3웨이브), 운명의 탑(2웨이브)
- 웨이브별 보스 강화: 1.5x → 2.0x → 3.0x (엘리트)
- 빨간 비네트 맥동, 보스 입장 바운스 애니메이션
- 웨이브 간 20% HP 회복, 폭발 파티클 축하
- 클리어 보상: 골드 + 경험치 + 보너스 아이템

### 32-2. 던전 내 상점 (Phase 273)
- 날짜: 2026-02-24
- 켈레온 상점: 층 깊이별 티어 아이템 (4-8개)
- 탄 카펫 비주얼 + 가격표 + 골드 HUD
- 구매/판매 오버레이 UI
- 도둑 시스템: 미결제 시 강력한 보안 몬스터 스폰
- 보라색 미니맵 표시

### v4.0.0 마일스톤 🎉
- **보스 가운틀릿**: 멀티 웨이브 보스전 + 비네트 + 파티클
...
## Phase 35: 퀘스트 + NPC 시스템

### 35-1. 퀘스트/미션 보드 (Phase 281)
- 날짜: 2026-02-25
- 일일 퀘스트 3개 (날짜 시드 기반 결정론적 생성)
- 도전 퀘스트 8개 (영구, 더 어려움, 높은 보상)
- 퀘스트 보드 씬: Daily/Challenges 탭, 진행도 바, 수령 버튼
- 던전 실행 중 자동 진행 추적 (킬, 아이템, 체인, 보스)
- 허브에 빨간 알림 배지

### 35-2. 마을 NPC 대화 (Phase 282)
- 날짜: 2026-02-25
- 6종 NPC: Chansey, Kangaskhan, Kecleon, Absol, Pelipper, Lucario
- 진행도 반응 대사 (클리어 수, NG+, 스토리지, 퀘스트 상태)
- 타자기 텍스트 효과 (30ms/글자)
- NPC 아바타 스트립 + 아이들 애니메이션
...
## Phase 37: 렐릭 아티팩트 + 런 로그

### 37-1. 렐릭 아티팩트 시스템 (Phase 287)
- 날짜: 2026-02-25
- 15종 렐릭 (4등급: Common/Rare/Epic/Legendary)
- 런당 최대 3개, 보스/가운틀릿/전설 처치 시 드롭
- ATK/DEF/HP/크리/흡혈/재생/포커스 새시 등 효과
- 렐릭 선택 오버레이 + HUD 아이콘 + 등급별 VFX

### 37-2. 전투 로그 + 런 서머리 (Phase 288)
- 날짜: 2026-02-25
- 20종 이벤트 로깅 (킬, 데미지, 아이템, 스킬, 층, 발견 등)
- 퍼포먼스 등급 (D~SSS) 자동 계산
- 클리어/게임오버 시 "Run Summary" 오버레이
- 핵심 통계 + 주요 이벤트 + 층별 타임라인

### v4.5.0 마일스톤
- **렐릭**: 15종 4등급 런 아티팩트 + 전투 통합
- **런 로그**: 이벤트 추적 + 퍼포먼스 등급 + 서머리
- **라이브**: https://songclaude-bot.github.io/poke-roguelite/
...
### 38-1. 축복 & 저주 시스템
- 날짜: 2026-02-25
- 5종 축복: 전사의 영혼(ATK+20%), 수호자의 방패(DEF+20%), 행운의 가호(골드+30%), 신속한 발걸음(회피 15%), 자연의 은총(리젠)
- 5종 저주: 허약한 몸(HP-15%), 무거운 짐(배고픔+20%), 실명(시야-2), 불운(골드-20%), 마나 유출(PP+1)
- 5층마다 랜덤 축복/저주, 퍼즐 성공=축복, 실패=저주
- HUD 아이콘 + 터치 정보 팝업 + 획득 애니메이션

### 38-2. 적 변형 시스템
- 5종: Normal, Elite, Shiny, Shadow, Ancient
- 층/난이도 기반 등장 확률, 고유 색상 틴트 + 스케일

### v4.6.0 마일스톤
- **축복/저주**: 10종 임시 버프/디버프 + 10가지 효과 통합
- **적 변형**: 5종 특수 적 + 비주얼 + 보상
- **라이브**: https://songclaude-bot.github.io/poke-roguelite/
...
### 39-1. 던전 신전 시스템
- 날짜: 2026-02-25
- 6종 신전: Blessing, Sacrifice, Gamble, Healing, Skill, Mystery
- 플레이어 선택지 기반 상호작용 (2-3개 옵션)
- 미니맵 통합, 15% 스폰 확률, 시각 효과

### 39-2. 장비 강화 대장간
- 날짜: 2026-02-25
- 5단계 강화 (골드 100~2000), 보너스 +10%~+70%
- ForgeScene: 별 레벨 표시, 모루 아스키아트
- 지닌 아이템 스탯 보너스에 강화 배율 적용
...
## Phase 40: 동료 진화 + 상태이상 확장

### 40-1. 동료 진화 시스템
- 날짜: 2026-02-25
- 동료 50% 경험치 공유, 던전 내 레벨업
- 진화 조건 충족 시 자동 진화 (흰색 플래시 + 스프라이트 갱신)
- 진화 시 스탯 보너스 + 새 기술 학습 가능

### 40-2. 상태이상 확장
- 날짜: 2026-02-25
- 5종 추가: Frozen(빙결), Badly Poisoned(맹독), Flinch(풀죽음), Drowsy(졸음), Cursed(저주)
- 빙결: 2-3턴 행동불가, 20% 해빙, 불 공격 즉시 해빙
- 맹독: 매턴 증가하는 피해 (1,2,3...)
- 풀죽음: 1턴 스킵 후 자동 제거
- 졸음: 3턴간 30% 확률로 잠듦
- 저주: 8턴간 4턴마다 최대 HP 25% 피해
- HUD 아이콘 + 틴트 컬러 + 주기적 플래시

### v4.8.0 마일스톤
- **동료 진화**: 던전 내 동료 레벨업/진화 시스템
- **상태이상**: 5종 신규 (빙결/맹독/풀죽음/졸음/저주)
- **라이브**: https://songclaude-bot.github.io/poke-roguelite/

## Phase 41: 구조 시스템 + 타입 젬

### 41-1. 던전 구조 시스템
- 날짜: 2026-02-25
- 기절 시 골드로 부활 (런당 최대 2회)
- 기본 구조: 50% HP, 아이템 절반 소실
- 프리미엄 구조: 100% HP, 아이템 유지 (3배 비용)
- 비용: 층수/난이도 기반 스케일링
...
- 18종 타입 젬 (소모품, 층별 효과)
- +50% 해당 타입 기술 데미지 부스트
- 바닥 아이템으로 출현, 일반 타입 가중치 높음
- 기본 공격 + 스킬 공격 모두 적용
- HUD 아이콘 표시

### v4.9.0 마일스톤
- **구조**: 기절 시 골드 부활 시스템
- **타입 젬**: 18종 타입별 +50% 데미지 부스트
- **라이브**: https://songclaude-bot.github.io/poke-roguelite/

## Phase 42: 층 이벤트 + 업적 확장

### 42-1. 층 이벤트 시스템
- 날짜: 2026-02-25
- 9종 이벤트: Treasure, Foggy, Windy, Gravity, Inverse, ZeroIsland, Lucky, Famine, Warp
- 층 진입 시 30% 확률 발동, 난이도/층수 기반 가중치
- TreasureFloor: 2배 아이템/골드, InverseFloor: 상성 반전
- WindyFloor: 넉백, WarpFloor: 적 10% 텔레포트
- FamineFloor: 아이템 없음 + 2배 배고픔 소모

### 42-2. 업적 확장 (36종)
- 날짜: 2026-02-25
- 15종 신규: 신전 탐색가, 축복받은 영혼, 저주 파괴자, 마스터 대장장이
- 젬 수집가, 구조 레인저, 변형 사냥꾼, 그림자 슬레이어
- 고대 정복자, 체인 마스터, 퍼즐 마스터, 풀 하우스
- 진화 전문가, 스피드 데몬
- 14종 새 저장 필드 + 역호환성 보장

### v5.0.0 마일스톤
- **v5.0.0 대규모 마일스톤!**
- **층 이벤트**: 9종 글로벌 층 효과 시스템
- **업적**: 36종 (기존 21 + 신규 15)
- **축복/저주**: 10종 런 버프/디버프
- **신전**: 6종 인터랙티브 신전
- **대장간**: 5단계 장비 강화
...
## Phase 43: 날씨 예보 + 파티 프리셋

### 43-1. 날씨 예보 시스템
- 날짜: 2026-02-25
- DungeonPreview에서 다음 5층 날씨 예측
- 던전 내 HUD에 다음 2층 날씨 표시
- 강도 표시 (!/!!)
...
### 45-1. 미니보스 시스템
- 날짜: 2026-02-25
- 8종 미니보스 풀 (Scyther, Gengar, Machamp 등)
- 10% 스폰 확률, 2-3배 HP, 오렌지 HP 바
- 보장 보상: 골드/축복/유물/아이템

### 45-2. 탐색 보상 시스템
- 날짜: 2026-02-25
- 3단계: Explorer(50%), Cartographer(75%), Trailblazer(100%)
- 골드/경험치 보너스, 애니메이션 배너
- 미니맵 텍스트 색상 변화
